...
class DurationNetwork(nn.Module):
    def __init__(self,
                 embedding_size, embedding_audio_size, hidden_size, vocab_size,
                 device, ms_per_step, duration_scale, dropout=0.1):
        super().__init__()
        ...
        self.encoder = Encoder(
            hidden_size=embedding_size, embedding_size=embedding_size,
        ...
        self.decoder_audio = Decoder(
            embedding_size, hidden_size, output_size=hidden_size,
            num_layers=2, dropout=dropout, time_scale=None)
    ...
    def forward(self, features_transcription, mask, features_audio, masks_audio):
        ...
        total_duration = masks_audio.sum(1) * self.ms_per_step if masks_audio is not None else features_audio.shape[
                                                                                                   1] * self.ms_per_step
        if mask is not None:
