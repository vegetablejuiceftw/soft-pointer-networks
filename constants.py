...
DURATION_SCALER = 256.0  # duration models do not play well with 123ms bot not also with 0.1sec ...
...
KNOWN_LABELS = list(
    sorted(
        set(
            TRANSFORM_MAPPING.get(k, k) for k in sorted(FOUND_LABELS.keys())
            if TRANSFORM_MAPPING.get(k, k))))
KNOWN_LABELS_COUNT = len(KNOWN_LABELS)

MAP_LABELS = {
    label: (
        [
            int(KNOWN_LABELS.index(label) == i)
            for i in range(KNOWN_LABELS_COUNT)
        ],
